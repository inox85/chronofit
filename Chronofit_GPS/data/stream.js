...
const lineColorsDark = {
  1: "#c0393a", 2: "#2a8c4a", 3: "#3a50c0", 4: "#a07820", 5: "#555555"
};
...
function onThemeCheckChange(chk) {
  ...
  // non salva qui: si salva su "Applica"
}

// ── View mode ─────────────────────────────────────────────────
// anagrafica | griglia | entrambi
let currentViewMode = "anagrafica";

function applyViewMode(mode) {
  currentViewMode = mode;
  document.body.classList.remove("view-anagrafica", "view-griglia", "view-entrambi");
...
function addEventToTable(rowIndex, lineNumber, competitor, h, m, s, ms) {
  ...
  // Normalizza tutti i valori numerici per evitare "undefined" nelle celle
  const ri   = rowIndex   ?? "?";
  ...
  const lineColor = lineColors[ln] || "#ccc";
  ...
  try {
    ...
    text.trim().split("\n").forEach(line => {
      ...
      try {
        ...
        // Salta entry malformate o senza i campi minimi attesi
...
// ── Prefs ─────────────────────────────────────────────────────
const STREAM_PREFS_KEY = "chronofit_stream_prefs";
let activeStreamUrl  = "";
...
function restoreStreamPrefs() {
  const raw = localStorage.getItem(STREAM_PREFS_KEY);
  if (!raw) return;
  try {
    ...
    if (prefs.monitorLine) {
      ...
      updateLineBarColor(parseInt(prefs.monitorLine));
    }
...
// Mostra l'indicatore "tocca per avviare" quando c'è un URL salvato
...
function loadStream(url, type) {
  ...
  console.log(`▶ Stream [${resolved}]:`, url);
  ...
  if (resolved === "mjpeg") {
    ...
    // Preferisce hls.js (più affidabile su Chrome/Android/Firefox).
    // Usa il playback nativo solo come fallback (Safari/iOS).
    if (typeof Hls !== "undefined" && Hls.isSupported()) {
      ...
      // Fallback nativo — Safari / iOS
      video.src = url; video.load();
      video.play().catch(e => console.warn("Autoplay bloccato:", e));
    } else {
      showStreamError("HLS non supportato: hls.js non caricato e nessun supporto nativo");
    }
  } else {
    video.style.display = "block";
    video.src = url; video.load();
...
function applySettings() {
  // ── stream: carica sempre se c'è un URL (click su Applica = gesto utente) ──
  ...
  // ── linea: reset card se selezionata ────────────────────────
  updateLineBarColor(parseInt(document.getElementById("lineSelect").value));
  resetToWaiting();
  ...
  timePrecision = prec;
  // ricalcola i tempi nella tabella con la nuova precisione
  document.querySelectorAll("#event-table tbody tr").forEach(row => {
    const tsCell = row.querySelector(".timestamp");
    if (!tsCell) return;
    const h  = parseInt(row.dataset.hour    ?? 0);
  ...
  if (raw) {
    try {
      ...
      return; // non chiudere se c'è un errore
    }
  }
...
// file picker: rileva csv vs json e popola textarea
...
// Tap sul placeholder → avvia stream salvato (gesto utente = autoplay permesso)
// Nota: al caricamento iniziale video.style.display è "" (dal foglio CSS), non "none",
// quindi si controlla se è esplicitamente "block" (impostato da loadStream).
...
function resetToWaiting() {
  ...
  card.classList.remove("state-arrived", "arrival-anim");
  card.classList.add("state-waiting");
  const statusEl = document.getElementById("compLineStatus");
...
// status: "PROSSIMO" (competitor assegnato, non ancora transitato)
//         "TRANSITO" (checkpoint fisico scattato)
function displayAthlete(competitorNum, lineNum, data, status) {
  const athlete = findAthlete(competitorNum);
  const card    = document.getElementById("competitorCard");

  updateLineBarColor(lineNum);

  const statusEl = document.getElementById("compLineStatus");
  statusEl.textContent = status;
  statusEl.classList.toggle("status-prossimo", status === "PROSSIMO");

  // 0 è falsy in JS → confronto esplicito
  document.getElementById("compNumber").textContent =
  ...
  if (athlete) {
    ...
    const extra = athlete.nationality || athlete.nazione   || athlete.nation ||
                  athlete.country     || "";
    document.getElementById("compName").textContent      = lname ? lname.toUpperCase() : (fname ? fname.toUpperCase() : "—");
    document.getElementById("compFirstName").textContent = lname ? fname : "";
    document.getElementById("compTeam").textContent      = team;
    document.getElementById("compCategory").textContent  = cat;
    document.getElementById("compExtra").textContent     = extra;
  } else {
    document.getElementById("compName").textContent      = "—";
    document.getElementById("compFirstName").textContent = "";
    document.getElementById("compTeam").textContent      = "";
    document.getElementById("compCategory").textContent  = "";
    document.getElementById("compExtra").textContent     = "";
  }

  // Orario: solo per TRANSITO (checkpoint fisico); per PROSSIMO mostra "—"
  if (status === "TRANSITO" && data) {
    ...
    document.getElementById("compTime").textContent = `${h}:${m}:${s}.${ms}`;
  } else {
    document.getElementById("compTime").textContent = "—";
  }

  card.classList.remove("state-waiting", "arrival-anim");
  void card.offsetWidth; // reflow per riavviare l'animazione
  card.classList.add("state-arrived", "arrival-anim");
  setTimeout(() => card.classList.remove("arrival-anim"), 900);

  // Auto-reset solo per TRANSITO (dopo 60 s); PROSSIMO resta fino al prossimo evento
  if (clearArrivedTimer) { clearTimeout(clearArrivedTimer); clearArrivedTimer = null; }
  if (status === "TRANSITO") {
...
function updateLineBarColor(lineNum) {
  const bar   = document.getElementById("compLineBar");
  const color = lineColors[lineNum]     || "#ccc";
  const dark  = lineColorsDark[lineNum] || "#555";
  bar.style.background = color;
  bar.style.color      = dark;
  document.getElementById("compLineLabel").textContent = "LINE " + lineNum;
  document.getElementById("competitorCard").style.borderColor = color;
}
...
function handleMessage(data) {
  ...
  switch (data.t) {
    ...
    // Checkpoint fisico: atleta ha attraversato la linea
    case TYPE_CHECKPOINT:
      // Aggiorna la card solo se è la linea monitorata
    ...
    // data.l  = numero linea (1-4)
    // data.c  = numero competitor  (0 = nessuno assegnato)
    // data.e  = linea abilitata (0/1)
    case TYPE_LINE_UPDATED:
      if (parseInt(data.l) === sel) {
        if (data.e === 0) {
          // linea disabilitata → torna in attesa
          resetToWaiting();
        } else if (parseInt(data.c) !== 0) {
          // nuovo competitor valido → mostra "PROSSIMO"
...
function updateClock(data) {
  ...
  const m = String(data.m ?? 0).padStart(2, "0");
  const s = String(data.s ?? 0).padStart(2, "0");
  document.getElementById("stream-clock").textContent = `${h}:${m}:${s}.000`;
}
...
function initResizablePanel() {
  ...
  const body      = document.querySelector(".stream-body");
  if (!handle || !sidePanel || !body) return;
  ...
  function onStart(clientX) {
    ...
    // Blocca la transizione di width durante il drag
    sidePanel.style.transition = "none";
  }

  function onMove(clientX) {
    if (!dragging) return;
    const dx      = startX - clientX;          // positivo = sposta verso destra → panel più largo
    const newW    = Math.max(PANEL_MIN, Math.min(PANEL_MAX, startW + dx));
    sidePanel.style.width = newW + "px";
    // In modalità griglia / entrambi, override la larghezza CSS body class
    sidePanel.style.setProperty("width", newW + "px", "important");
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Table column overlay — header click + live changes
